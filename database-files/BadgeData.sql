insert into Badge (Name, PointsWorth) values ('Adventurer', 60);
insert into Badge (Name, PointsWorth) values ('Music Lover', 11);
insert into Badge (Name, PointsWorth) values ('Eco-friendly', 25);
insert into Badge (Name, PointsWorth) values ('Player One', 22);
insert into Badge (Name, PointsWorth) values ('Book Worm', 41);
insert into Badge (Name, PointsWorth) values ('Film Buff', 11);
insert into Badge (Name, PointsWorth) values ('Gym Rat', 32);
insert into Badge (Name, PointsWorth) values ('Tech Enthusiast', 15);
insert into Badge (Name, PointsWorth) values ('Foodie', 15);
insert into Badge (Name, PointsWorth) values ('Fashionista', 17);
INSERT INTO Badge (BadgeId, Name, PointsWorth)
VALUES
(1, 'Community Builder', 70),
(2, 'Event Enthusiast', 50);